...
```bash
## 수정하려는 helm app 이 dailyfeed-activity 일 경우
cd dailyfeed-app-helm
cd activity
source uninstall-helm.sh ## dailyfeed-activity 제거
source install-helm-local.sh ## dailyfeed-activity 실행 
```
...
```bash
# 새로운 환경에서 클론 (clone)
git clone --recurse-submodules http://github.com/alpha3002025/dailyfeed-activity-svc  

# 또는 클론 후 submodule 초기화
git clone dailyfeed-activity-svc
cd dailyfeed-activity-svc
git submodule init
